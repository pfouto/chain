...
import ringpaxos.messages.DecisionMsg;
import ringpaxos.messages.DecidedMsg;
import ringpaxos.messages.AcceptedMsg;
import ringpaxos.messages.PrepareMsg;
import ringpaxos.messages.PrepareOkMsg;
import ringpaxos.messages.ReqAcceptMsg;
import ringpaxos.messages.ReqDecisionMsg;
import ringpaxos.timers.LeaderTimer;
import ringpaxos.timers.NoOpTimer;
import ringpaxos.timers.ReconnectTimer;
...
public class RingPaxosPiggyProto extends GenericProtocol implements MessageListener<BabelMessage> {
    ...
    private void sendNextAccept(PaxosValue val) {
        ...
        List<DecisionMsg> decs = new LinkedList<>();
        while (!pendingDecs.isEmpty()) decs.add(pendingDecs.remove());
        AcceptDecMsg acceptMsg = new AcceptDecMsg(instance.iN, currentSN.getValue(), nextValue, decs);
    ...
    private void uponAcceptMsg(AcceptDecMsg msg, Host from, short sourceProto, int channel) {

        msg.decisionMsgs.forEach(m -> uponDecisionMsg(m, from, sourceProto, channel));
    ...
    private void uponReqAcceptMsg(ReqAcceptMsg msg, Host from, short sourceProto, int channel) {
        ...
        if (inst.acceptedValue != null) {
            sendMessage(new AcceptDecMsg(inst.iN, inst.highestAccept, inst.acceptedValue,
                    Collections.emptyList()), from);
        }
    }
