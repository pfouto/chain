...
public class EPaxosProto extends GenericProtocol {
    ...
    private final Map<Host, HashMap<Integer, Instance>> instances = new HashMap<>();
    ...
    public EPaxosProto(Properties props, EventLoopGroup workerGroup) throws UnknownHostException {
        ...
        seeds.forEach(h -> instances.put(h, new HashMap<>(INITIAL_MAP_SIZE)));
    ...
    private void startNextInstance(PaxosValue nextValue) {
        ...
        for (Map.Entry<Host, HashMap<Integer, Instance>> entry : instances.entrySet()) {
    ...
    private boolean strongConnect(GraphNode v, BoxedInteger index, Stack<GraphNode> stack,
        ...
        for (Map.Entry<Host, Integer> e : instances.get(v.replica).get(v.iN).getDeps().entrySet()) {
